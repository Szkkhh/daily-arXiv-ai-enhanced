...
TL;DR: 本文提出Policy-First Tooling，一种模型无关的工具调用权限层，通过显式策略约束、风险感知门控、恢复控制与可审计解释来提升工具使用安全性；实验表明其在违规预防、重试放大抑制和敏感信息泄漏召回方面显著优于基线，但以任务成功率下降为代价。


<details>
  <summary>Details</summary>
Motivation: 现有工具使用自动化系统（如脚本、CI机器人、智能体助手）存在重复性安全失败模式（如不安全副作用、非法参数、失控重试、敏感输出泄露），且主流模型中心化/提示依赖型缓解方案缺乏泛化性与鲁棒性。

Method: 提出Policy-First Tooling框架：包含紧凑型策略DSL、支持可操作归因与修复提示的运行时执行架构，以及基于轨迹回放、可控故障与误用注入的可复现基准测试方法。

Result: 在225次受控实验中（5个策略包×3类故障），最严格策略包P4相较基线P0将违规预防率从0.000提升至0.681，重试放大系数从3.774降至1.378，敏感信息泄漏召回率达0.875；但任务成功率由0.356降至0.067。

Conclusion: Policy-First Tooling实现了工具调用安全性的显式建模与可量化权衡，验证了策略驱动、模型无关的防护范式在提升鲁棒性与可观测性方面的有效性，同时揭示了安全与效用间的本质张力。
...
TL;DR: 本文提出一种结合模糊推理与认证加密的自适应图像隐写框架，根据局部熵、边缘强度和负载压力动态选择1-3比特嵌入深度，并通过低位剥离图像保持编解码同步，同时采用Argon2id与AES-256-GCM保障载荷机密性与完整性。


<details>
  <summary>Details</summary>
Motivation: 固定深度LSB嵌入虽简单高效，但对平滑区与纹理区统一修改，导致统计可检测性升高；需在容量、视觉保真度与统计不可检测性间取得更好平衡。

Method: 构建Mamdani型模糊推理系统，以局部熵、边缘幅值和负载压力为输入语言变量，输出像素级1–3比特嵌入深度；特征图基于低位剥离图像计算，确保自适应机制对LSB修改鲁棒；叠加Argon2id密钥派生与AES-256-GCM认证加密层。

Result: 在保持高嵌入容量的同时显著降低统计失真，提升对SPAM、SRM等主流隐写分析器的抗检测能力；编解码同步稳定，无需额外辅助信息；加密层独立保障载荷机密性与完整性。

Conclusion: 该方法实现了嵌入策略自适应性与密码学安全性双重增强，为高容量低可检测性图像隐写提供了新范式。
...
TL;DR: This paper proposes a new paradigm for ensuring the reliability, safety, and security of large language model (LLM) agents deployed as autonomous 'AI scientists', addressing critical gaps in current safety benchmarks through a science-specific threat taxonomy, automated adversarial benchmark generation via multi-agent systems, and a multilayered defense framework integrating red-teaming, external controls, and an internal Safety LLM Agent.


<details>
  <summary>Details</summary>
Motivation: Existing general-purpose safety benchmarks suffer from domain mismatch, insufficient coverage of science-specific threats (e.g., biosafety risks, dangerous physical/biological outcomes), and benchmark overfitting—creating a critical vulnerability evaluation gap for LLMs operating autonomously in scientific domains.

Method: The paper employs three core methodological components: (1) synthesis of a science-contextualized taxonomy of LLM threats; (2) conceptualization of a multi-agent system for automated generation of domain-specific adversarial security benchmarks; and (3) design of a multilayered defense framework unifying red-teaming, external boundary controls, and an internal Safety LLM Agent.

Result: A structured, science-aware framework for evaluating and mitigating LLM agent risks—including reliability (factual accuracy, reproducibility), safety (prevention of physical/biological harm), and security (resistance to malicious misuse)—is established, enabling more rigorous and domain-appropriate assurance of trustworthy deployment.

Conclusion: Trustworthy deployment of LLM agents in scientific research necessitates moving beyond generic safety benchmarks toward domain-specific evaluation infrastructures and integrated, layered defense mechanisms—paving the way for responsible AI-driven scientific discovery.
...
TL;DR: PlanTwin is a privacy-preserving architecture that enables cloud-hosted LLMs to perform planning over private local environments without exposing raw context, by using a schema-constrained, de-identified digital twin and enforcing disclosure budgets via a local gatekeeper.


<details>
  <summary>Details</summary>
Motivation: Cloud-based LLM planners require access to local environment state (e.g., source code, credentials), but exposing raw context violates privacy; existing solutions fail to restrict what the cloud planner observes during planning.

Method: Introduces PlanTwin: (1) constructs a planning-oriented digital twin — an abstract, schema-constrained, de-identified graph preserving only planning-relevant structure; (2) restricts cloud planner interaction to a bounded capability interface over the twin; (3) deploys a local gatekeeper enforcing safety policies and cumulative disclosure budgets; (4) formalizes privacy-utility trade-off via capability granularity and defines privacy goals using (k,δ)-anonymity and ε-unlinkability; (5) mitigates multi-turn compositional leakage via disclosure control.

Result: Evaluated on 60 agentic tasks across 10 domains with 4 cloud planners: achieves perfect sensitive-item non-disclosure (SND = 1.0); three planners attain planning quality score (PQS) > 0.79; end-to-end utility loss < 2.2% compared to full-context baselines.

Conclusion: PlanTwin effectively decouples planning capability from raw context exposure, enabling provably private, high-fidelity cloud-assisted planning for sensitive local environments.
...
### [12] [CNT: Safety-oriented Function Reuse across LLMs via Cross-Model Neuron Transfer](https://arxiv.org/abs/2603.18449)
*Yue Zhao,Yujia Gong,Ruigang Liang,Shenchen Zhu,Kai Chen,Xuejing Yuan,Wangjun Zhang*

Main category: cs.CR

TL;DR: This paper proposes Cross-Model Neuron Transfer (CNT), a post-hoc method that transfers safety-related neuron subsets from donor LLMs to target LLMs, enabling modular, low-cost safety adaptation without retraining.


<details>
  <summary>Details</summary>
Motivation: The growing diversity of open-source LLMs with built-in safety features motivates reusing rather than rebuilding safety functionality, avoiding expensive data collection and training.
...
Conclusion: CNT is a general, practical, and efficient post-hoc safety adaptation framework that leverages cross-model neuron reuse for modular, low-overhead safety tuning.
...
### [13] [Impact of Differentials in SIMON32 Algorithm for Lightweight Security of Internet of Things](https://arxiv.org/abs/2603.18455)
*Jonathan Cook,Sabih ur Rehman,M. Arif Khan*

Main category: cs.CR

TL;DR: 本文提出了一种针对SIMON32分组密码的差分特性分析方法，通过构建部分差分分布表（PDDT）识别高概率差分路径，提升了可攻击轮数，超越现有最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有SIMON密码的差分密码分析效率低，主因是完整差分分布表（DDT）规模过大且高概率差分路径稀缺。

Method: 构建SIMON32的部分差分分布表（PDDT），系统性搜索并验证高概率差分特征，结合差分传播分析与实验验证。

Result: 发现了新的高概率差分路径，成功将SIMON32可攻击轮数提升至超过当前SOTA水平；开源了分析代码。

Conclusion: PDDT策略有效缓解了DDT规模与稀疏性瓶颈，为轻量级密码的高效差分分析提供了可行新范式。
...
### [14] [Quantifying Memory Cells Vulnerability for DRAM Security](https://arxiv.org/abs/2603.18549)
*Zilong Hu,Hongming Fei,Prosanta Gope,Jack Miskelly,Owen Millwood,Biplab Sikdar*

Main category: cs.CR

TL;DR: 本文提出了一种基于物理电荷泄漏与扰动路径的DRAM单元级电路建模框架，用于定量评估其在易失性、完整性与保密性三方面的安全特性，并揭示了漏洞分布的非均匀性与上下文依赖性。


<details>
  <summary>Details</summary>
Motivation: DRAM单元因保留失效、延迟变化、Rowhammer和Rowpress等现象导致位翻转，既可被用于攻击（破坏数据完整性与机密性），也可被用于安全机制（如PUF）；但若攻击者掌握单元级翻转脆弱性知识，则可能预测数据内容，威胁安全系统。因此需建立可量化、可解释的单元级脆弱性模型。

Method: 构建一个定量的、单元级的电路框架，直接从DRAM物理电荷泄漏与扰动路径建模单元脆弱性，并将器件层行为映射至系统级安全属性（易失性、完整性、保密性），结合典型失效模式进行实证分析。

Result: 框架成功揭示DRAM单元脆弱性具有非均匀分布和上下文依赖特征，支持对不同安全应用场景下DRAM适用性的系统化评估。

Conclusion: 该框架为DRAM在安全敏感应用（如密钥生成、PUF）中的可靠性评估提供了理论基础与实用工具，弥合了器件物理行为与系统安全属性之间的建模鸿沟。
...
### [15] [A Complexity Hierarchy of Shuffles in Card-Based Protocols](https://arxiv.org/abs/2603.18608)
*Tomoki Ono,Suthee Ruangwises*

Main category: cs.CR

TL;DR: This paper introduces a hierarchical classification of shuffle operations in card-based cryptography based on implementation complexity, proves theoretical separations between levels, and proposes a new protocol complexity measure grounded in this hierarchy.


<details>
  <summary>Details</summary>
Motivation: To address the practical implementability gap among existing card-based cryptographic shuffles and to establish a theoretically grounded framework for comparing their computational power.

Method: Classification of shuffle operations into complexity levels; formal proof of separation results showing certain shuffles cannot be realized using only lower-level operations; proposal of a new complexity measure for card-based protocols based on the hierarchy.

Result: A well-defined hierarchy of shuffle complexity levels is established; separation theorems are proven between key levels; a novel complexity measure for evaluating card-based protocols is introduced.

Conclusion: The hierarchy provides both practical guidance for protocol implementation and theoretical insight into the relative power of shuffle operations, enabling more principled design and analysis of card-based secure computation protocols.
...
### [16] [Cyber-Resilient Digital Twins: Discriminating Attacks for Safe Critical Infrastructure Control](https://arxiv.org/abs/2603.18613)
*Mohammadhossein Homaei,Iman Khazrak,Rubén Molano,Andrés Caro,Mar Ávila*

Main category: cs.CR

TL;DR: This paper proposes i-SDT, an intelligent self-defending Digital Twin for Industrial Cyber-Physical Systems that integrates physics-informed TCNs, MMD-based multi-stage attack discrimination, and uncertainty-aware MPC to enable real-time, shutdown-free resilient control under cyber-attacks.


<details>
  <summary>Details</summary>
Motivation: Existing DT-based anomaly detection methods cannot distinguish attack types and often resort to costly full-system shutdowns, leaving ICPS vulnerable to sophisticated sensor/actuator attacks.

Method: i-SDT combines (1) hydraulically-regularized Temporal Convolutional Networks with differentiable conservation constraints for robust nominal dynamics modeling; (2) a recurrent residual encoder with Maximum Mean Discrepancy (MMD) loss for latent-space separation of normal, single-stage, and multi-stage attacks; and (3) Model Predictive Control leveraging uncertainty-aware DT predictions for adaptive, shutdown-free resilient operation.

Result: Evaluated on SWaT and WADI datasets: significant improvement in detection accuracy, 44.1% reduction in false alarms, 56.3% lower operational costs in simulation-in-the-loop, and sub-second inference latency—demonstrating real-time feasibility on plant-level workstations.

Conclusion: i-SDT advances autonomous, operationally resilient cyber-defense for ICPS by unifying physics-informed learning, fine-grained attack discrimination, and uncertainty-aware control—eliminating the need for disruptive shutdowns while maintaining safety and performance.
...
TL;DR: MOSAIC is a multi-objective, slice-aware iterative curation framework for supervised fine-tuning that jointly optimizes safety alignment, low over-refusal on benign boundary queries, and instruction following—achieving Pareto-improved performance under fixed 1M-token budget across three evaluation axes.


<details>
  <summary>Details</summary>
Motivation: Balancing three competing objectives—multi-turn safety alignment, minimizing over-refusal on benign boundary queries, and instruction following under verifiable constraints—under a fixed supervised fine-tuning budget.

Method: Proposes MOSAIC: a closed-loop data mixture search framework built on a unified L1–L3 evaluation interface; converts slice-level failure profiles into executable data actions (dataset mixture ratios, bucket weights, focus criteria); performs five independent fine-tuning rounds from the same base model.

Result: Under 1M-token budget, MOSAIC improves XGuard (safety) from 2.76 to 4.67, maintains OrBench (over-refusal) at 4.41, and IFEval (instruction following) at 3.65; final Pareto solution outperforms random static LoRA baseline on independent attack, over-refusal, and capability tests.

Conclusion: Structured, slice-level failure diagnosis serves as an effective control signal for budget-constrained, multi-objective data curation—enabling generalizable alignment improvements without sacrificing instruction-following or over-refusal robustness.
...
### [19] [Cross-Ecosystem Vulnerability Analysis for Python Applications](https://arxiv.org/abs/2603.18693)
...
### [20] [ClawTrap: A MITM-Based Red-Teaming Framework for Real-World OpenClaw Security Evaluation](https://arxiv.org/abs/2603.18762)
...
### [21] [SoK: Practical Aspects of Releasing Differentially Private Graphs](https://arxiv.org/abs/2603.18779)
*Nicholas D'Silva,Surya Nepal,Salil S. Kanhere*

Main category: cs.CR

TL;DR: This paper provides a systematic analysis and practical framework for applying Differential Privacy (DP) to graph data, addressing challenges in privacy-utility trade-offs, method comparability, and DP interpretability.


<details>
  <summary>Details</summary>
Motivation: Graph data poses unique privacy risks due to sensitive edges and node attributes; existing DP methods for graphs are heterogeneous in definitions, goals, and settings, hindering fair comparison and sound deployment—especially for practitioners facing DP's interpretability challenges.

Method: The authors conduct a comprehensive survey of differentially private graph release methods, identify key vulnerabilities in current approaches, and propose an objective-based, practitioner-oriented framework for method selection, interpretation, and evaluation—validated via two social network analyst scenarios.

Result: A unified, adaptable systemisation of DP graph release methods is delivered, including a taxonomy of vulnerabilities and a benchmarking framework demonstrated on real-world social network use cases.

Conclusion: The proposed framework bridges the gap between theoretical DP guarantees and practical graph privacy deployment, enabling more informed, objective-driven, and empirically grounded choices among DP graph mechanisms.
...
### [22] [Functional Subspace Watermarking for Large Language Models](https://arxiv.org/abs/2603.18793)
*Zikang Ding,Junhao Li,Suling Wu,Junchi Yao,Hongbo Liu,Lijie Hu*

Main category: cs.CR

TL;DR: 提出Functional Subspace Watermarking (FSW)框架，将水印锚定于低维功能子空间，通过广义特征值分解、自适应谱截断与向量一致性约束，在保持模型语义性能前提下显著提升对微调、量化、知识蒸馏等参数扰动的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型水印方法在面对细粒度参数扰动（如微调、量化、知识蒸馏）时鲁棒性不足，因内部表征易受复杂失真影响，导致水印提取不可靠。

Method: 构建Functional Subspace Watermarking (FSW)框架：1）求解广义特征值问题提取稳定的功能子空间；2）引入自适应谱截断策略平衡鲁棒性与模型效用；3）施加向量一致性约束保障语义性能无损。

Result: 在多种LLM架构与数据集上验证，FSW在各类模型攻击下实现更高检测精度与统计可验证性，鲁棒性显著优于当前SOTA方法。

Conclusion: FSW通过功能子空间建模与结构化约束，为LLM水印提供了兼具强鲁棒性与高保真性的新范式，有效弥合了模型修改场景下的所有权保护鸿沟。
...
### [23] [Agent Control Protocol: Admission Control for Agent Actions](https://arxiv.org/abs/2603.18829)
*Marcelo Fernandez*

Main category: cs.CR

TL;DR: ACP is a formal governance protocol for autonomous agents in B2B settings, enforcing cryptographic admission control—verifying identity, capability scope, delegation chain, and policy compliance—before any action executes; it complements RBAC and Zero Trust, and v1.13 includes 36 documents across 5 conformance levels, a Go reference implementation, test vectors, OpenAPI spec, and >62 verifiable requirements.


<details>
  <summary>Details</summary>
Motivation: To enable safe, auditable, and institutionally governed operation of autonomous agents in B2B environments by preventing unauthorized or policy-violating state mutations through a cryptographically enforced admission control layer.

Method: Design and standardization of a layered, specification-driven protocol incorporating cryptographic identity, capability-based authorization, deterministic risk evaluation, verifiable chained delegation, transitive revocation, and immutable auditing—integrated atop existing access control models (RBAC, Zero Trust).

Result: ACP v1.13 specification: 36 technical documents across five conformance levels (L1–L5); Go reference implementation (22 packages, L1–L4); 51 Ed25519-signed conformance test vectors; OpenAPI 3.1.0 HTTP interface spec; >62 verifiable requirements and 12 prohibited behaviors; interoperability mechanisms across institutions.

Conclusion: ACP establishes a rigorous, implementable foundation for institutional control over autonomous agents—ensuring accountability, composability, and verifiability without sacrificing autonomy—making it suitable for high-assurance B2B automation and AI agent governance.
...
### [24] [Confidential Databases Without Cryptographic Mappings](https://arxiv.org/abs/2603.18836)
*Wenxuan Huang,Zhanbo Wang,Mingyu Li*

Main category: cs.CR

TL;DR: FEDB is a novel confidential database design that eliminates cryptographic operations from the critical path via crypto-free mappings, achieving up to 78× speedup on TPC-C/TPC-H.
...
Method: FEDB introduces crypto-free mappings—data-independent identifiers maintained inside the database that are securely mapped to plaintext secrets only within a trusted domain, thereby removing cryptography from the query execution critical path.

Result: Up to 78.0× reduction in runtime overhead on industry-standard TPC-C and TPC-H benchmarks.

Conclusion: Decoupling identifier management from cryptographic computation enables substantial performance gains without compromising confidentiality, advancing practical deployment of confidential databases.
...
### [25] [Security, privacy, and agentic AI in a regulatory view: From definitions and distinctions to provisions and reflections](https://arxiv.org/abs/2603.18914)
*Shiliang Zhang,Sabita Maharjan*

Main category: cs.CR

TL;DR: This paper reviews 24 EU AI regulatory documents (2024–2025) to clarify definitions of security, privacy, and agentic AI; disambiguates regulatory interpretations; synthesizes current provisions for AI types—especially concerning security and privacy—and reflects on aligning obligations with AI/agent behaviors to support governance and compliance.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of AI—particularly agentic AI—has outpaced regulatory clarity, especially in security and privacy domains where autonomous behaviors blur legal and technical boundaries.

Method: Systematic review and conceptual analysis of 24 EU regulatory documents published between 2024 and 2025; deconstruction and differentiation of key regulatory terms (security, privacy, agentic AI); synthesis of provisions across AI types; critical reflection on regulatory alignment with AI behaviors.

Result: Clarified, distinct definitions of security, privacy, and agentic AI within the EU regulatory context; synthesized mapping of current provisions targeting AI systems by type and security/privacy relevance; identification of misalignments between existing obligations and agentic AI behaviors.

Conclusion: Regulatory frameworks must evolve beyond static, system-centric models to accommodate dynamic, goal-directed agent behaviors; precise conceptual grounding is essential for effective AI governance, compliance, and cross-stakeholder coordination (policymakers, developers, researchers).
...
### [26] [Controller Datapath Aware Verification of Masked Hardware Generated via High Level Synthesis](https://arxiv.org/abs/2603.18939)
...
<details>
  <summary>Details</summary>
Motivation: HLS工具虽可自动将经验证的掩码软件综合为RTL硬件以降低设计开销，但其非安全导向的优化可能破坏PSCA安全性；而现有硬件掩码验证工具（如REBECCA）在处理HLS生成的控制器-数据通路架构时易产生误报。

Method: 提出MaskedHLSVerif工具流，对HLS生成的控制器-数据通路型掩码RTL进行逐状态（state-wise）形式化验证，显式建模控制逻辑与数据通路交互，消除因资源共享引发的状态混淆问题。

Result: 成功验证标准密码学基准（含级联掩码单元及掩码PRESENT S盒），规避了REBECCA等工具在相同设计上的误报；同时可检出由HLS优化（如寄存器复用、操作合并）导致的掩码实现缺陷。

Conclusion: 逐状态形式化验证是验证HLS生成掩码硬件PSCA安全性的有效范式；MaskedHLSVerif填补了面向HLS流程的安全验证空白，提升了自动化掩码硬件可信度。
...
### [27] [Security awareness in LLM agents: the NDAI zone case](https://arxiv.org/abs/2603.19011)
*Enrico Bottazzi,Pia Park*

Main category: cs.CR

TL;DR: This paper investigates how LLM agents assess the security of their execution environment—specifically whether they can reliably distinguish secure (e.g., TEE-based NDAI zones) from insecure environments—using evidence like attestation results. It finds that while all models suppress disclosure upon failed attestation (reliable danger detection), responses to successful attestation are highly inconsistent: some increase, some ignore, and some even decrease disclosure—revealing a fundamental inability to verify safety, critical for privacy-preserving agentic protocols.


<details>
  <summary>Details</summary>
Motivation: LLM agents lack native awareness of their execution environment and must infer security solely from contextual evidence; yet privacy-preserving protocols like NDAI zones require agents to reliably calibrate information disclosure based on verifiable security evidence.
...
Result: Universal suppression of disclosure under failing attestation; highly heterogeneous responses under passing attestation—including increased, unchanged, or paradoxically decreased disclosure—indicating robust danger detection but unreliable safety verification.

Conclusion: Current LLMs cannot reliably verify environmental safety despite detecting danger, exposing a critical gap for deploying trustworthy agentic protocols in secure enclaves; bridging it requires advances in interpretability, fine-tuning, or evidence representation design.
...
### [28] [On The Effectiveness of the UK NIS Regulations as a Mandatory Cybersecurity Reporting Regime](https://arxiv.org/abs/2603.19084)
...
### [29] [FedTrident: Resilient Road Condition Classification Against Poisoning Attacks in Federated Learning](https://arxiv.org/abs/2603.19101)
...
### [30] [Performance Testing of ChaCha20-Poly1305 for Internet of Things and Industrial Control System devices](https://arxiv.org/abs/2603.19150)
